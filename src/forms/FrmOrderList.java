...
import main.MyUtil;
import reports.ReportOrder;
import tablemodel.TmOrderContent;
import tablemodel.TmOrderList;
        
public class FrmOrderList extends javax.swing.JDialog {
    ...
    public FrmOrderList() {
        ...
        setTitle("Список заказов. " + conf.FORM_TITLE);
	...
	private void requery(){
        ...
        jTableDocList.setModel(new TmOrderList(getOrderListRS()));
        ...
        //"Док №","Дата","Примечание","К-во товаров","Статус", "Сумма"
...
//		}
	...
	private void requeryDocContent(BigDecimal docID) {
		...
		jTableDocContent.setModel(new TmOrderContent(getOrderContentRS(docID)));
	...
	private void jButtonPrintCheckActionPerformed() {
        ...
        int rowNum = jTableDocList.getRowSorter().convertRowIndexToModel(selectedRow);
        BigDecimal currentDocID = (BigDecimal) jTableDocList.getModel().getValueAt(rowNum, 0);
		ReportOrder reportOrder = new ReportOrder(currentDocID);
		reportOrder.setModal(true);
		reportOrder.setVisible(true);
	}
	private void jButtonDocStatusActionPerformed(){
        cnn = ConnectionDb.getInstance();
        if (cnn == null) return;
        int selectedRow = jTableDocList.getSelectedRow();
        if (selectedRow == -1) return;
        ...
        int i = JOptionPane.showConfirmDialog(null, "После изменения статуса документа\nизменять документ запрещено!\n\nОтправить менеджеру?", "ВНИМАНИЕ!", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
        if (i == 0) {
            cnn.setOrderStatus(currentDocID, 10);
            requery();
        }
    }
	...
	private ResultSet getOrderContentRS(BigDecimal docID) {
		cnn = ConnectionDb.getInstance();
		if (cnn == null) return null;
		ResultSet rs = cnn.getOrderContent(docID,"asc");
		return rs;
	}
    private ResultSet getOrderListRS() {
        cnn = ConnectionDb.getInstance();
        if (cnn == null) return null;
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd.MM.yy");
        ...
        try {
            ...
            //Logger.getLogger(FrmCardAttribute.class.getName()).log(Level.SEVERE, null, ex);
			MyUtil.errorToLog(this.getClass().getName(), ex);
            DialogBoxs.viewError(ex);
        }

        ResultSet rs = cnn.getOrderList(dt1,dt2);
        return rs;
    }
    ...
    private void initComponents() {
        ...
        jButtonDocOut = new javax.swing.JButton();
        ...
        jButtonDocEdit.setToolTipText("Изменить заказ");
        ...
        jButtonDocDel.setToolTipText("Удалить заказ");
        ...
        jButtonDocCopy.setToolTipText("Копировать заказ");
        ...
        jButtonDocOut.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        jButtonDocOut.setIcon(new javax.swing.ImageIcon(getClass().getResource("/png/Cash-register-32.png"))); // NOI18N
        jButtonDocOut.setText("Отправить менеджеру");
        jButtonDocOut.setToolTipText("Отправить менеджеру");
        jButtonDocOut.setActionCommand("Установить нач. ост.");
        jButtonDocOut.setBorderPainted(false);
        jButtonDocOut.setMaximumSize(new java.awt.Dimension(70, 70));
        jButtonDocOut.setMinimumSize(new java.awt.Dimension(70, 70));
        jButtonDocOut.setPreferredSize(new java.awt.Dimension(70, 70));
        jButtonDocOut.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonDocStatusActionPerformed(evt);
            }
        });
    ...
    private void jButtonDocStatusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDocStatusActionPerformed
        jButtonDocStatusActionPerformed();
    }//GEN-LAST:event_jButtonDocStatusActionPerformed
    ...
    private javax.swing.JButton jButtonDocOut;
