...
public class FrmPriceOverList extends javax.swing.JDialog {
	...
	private void jButtonCreateStickerActionPerformed() {
		...
		if (selectedRow == -1) return;
		currentDocID = jTableDocList.getModel().getValueAt(selectedRow, 0).toString();
		int i = JOptionPane.showConfirmDialog(null, "Создать ценники \nна основании переценки товара?", "ВНИМАНИЕ!", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
		if (i == 0) {
			BigDecimal bd_result = cnn.setCreateStickers(currentDocID,"PriceOver");
    ...
    private void initComponents() {
        ...
        jButtonCreateSticker.setToolTipText("Создать ценники");
