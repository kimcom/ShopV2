...
public class ReportReceipt extends JDialog implements Printable{
...
//		System.out.println("page: "+Integer.toString(pageNumber));
    ...
    public ReportReceipt(String docID) {
		currentDocID = docID;
    ...
    private void clientInfo() {
        ...
        setTitle("Приход. ".concat(conf.FORM_TITLE));
        ...
        jLabel6.setText("Приход №"+currentDocID+"   ("+MyUtil.getClientID()+")");
        jLabel30.setText(MyUtil.getCurrentDateTime());
    }
    private void fillCheck(){
        cnn = ConnectionDb.getInstance();
        if (cnn == null) return;
        ResultSet res = cnn.getReceiptReport(currentDocID);
        int y = 0, h = 15;
		try {
			while (res.next()) {
				//for (int i = 0; i < 5; i++) {
...
//System.out.println(res.getString("GoodID").toString()+"	"+str);
				BigDecimal bdReceipt = res.getBigDecimal("Quantity");
				String strReceipt = "";
				if (bdReceipt != null) 
					strReceipt = bdReceipt.setScale(2, RoundingMode.HALF_UP).toPlainString();
				jLabel13.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
				jLabel13.setText(strReceipt);
