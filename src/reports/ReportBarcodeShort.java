...
public class ReportBarcodeShort extends JDialog implements Printable{
...
//		System.out.println("page: "+Integer.toString(pageNumber));
		...
		if (blViewDialogPrint) {
			if (pj.printDialog()) {
				try {
					...
					//Logger.getLogger(ReportBarcodeShort.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public ReportBarcodeShort(int division) {
    ...
    private void clientInfo() {
        ...
        setTitle("Короткие ШК. ".concat(conf.FORM_TITLE));
        ...
        jLabel6.setText("КОРОТКИЕ ШК     (Отдел №"+Integer.toString(division)+")");
        jLabel30.setText(MyUtil.getCurrentDateTime());
    }
    private void fillCheck(){
        cnn = ConnectionDb.getInstance();
        if (cnn == null) return;
        ResultSet res = cnn.getBarcodeShortReport(division);
        int y = 0, h;
        try {
            while (res.next()) {
				//for (int i = 0; i < 5; i++) {
