...
public class ReportPriceOver extends JDialog implements Printable{
...
//		System.out.println("page: "+Integer.toString(pageNumber));
		...
		if (blViewDialogPrint) {
			if (pj.printDialog()) {
				try {
					...
					//Logger.getLogger(ReportPriceOver.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public ReportPriceOver(String docID) {
		currentDocID = docID;
    ...
    private void clientInfo() {
        ...
        setTitle("Переоценка. ".concat(conf.FORM_TITLE));
        ...
        jLabel6.setText("Переоценка №"+currentDocID+"   ("+MyUtil.getClientID()+")");
        jLabel30.setText(MyUtil.getCurrentDateTime());
    }
    private void fillCheck(){
        cnn = ConnectionDb.getInstance();
        if (cnn == null) return;
        ResultSet res = cnn.getPriceOverReport(currentDocID);
        int y = 0, h = 15;
		try {
			while (res.next()) {
				//for (int i = 0; i < 5; i++) {
...
//System.out.println(res.getString("GoodID").toString()+"	"+str);
				BigDecimal bdPriceOver = res.getBigDecimal("PriceOver");
				String strPriceOver = "";
				if (bdPriceOver != null) 
					strPriceOver = bdPriceOver.setScale(2, RoundingMode.HALF_UP).toPlainString();
				jLabel13.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
				jLabel13.setText(strPriceOver);
				jPanel2.add(jLabel13, new org.netbeans.lib.awtextra.AbsoluteConstraints(165, y, 45, h));
