...
public class ReportMarkup extends JDialog{
...
//System.out.println(""+i+" "+c.getWidth() + " " + c.getHeight());
    ...
    public ReportMarkup() {
		...
		dim.height = dim.height - 60;
		dim.width = getWidth() + 10;
		setSize(dim);
		clientInfo();
	...
	private String getColumnName(int col,int type){
		String res = "";
		if (col == 0 && type == 0) res = "Группировка по категориям наценки";
		if (col == 0 && type == 1) res = "Группировка по категориям наценки и товарам";
		if (col == 0 && type == 2) res = "Группировка по категориям наценки и сотрудникам";
		if (col == 0 && type == 3) res = "Группировка по сотрудникам и категориям наценки";
		if (col == 1 && type == 0) res = "№ п-п";
		if (col == 1 && type == 1) res = "Артикул";
		if (col == 1 && type == 2) res = "Должность";
		if (col == 1 && type == 3) res = "";
		if (col == 2 && type == 0) res = "Категория";
		if (col == 2 && type == 1) res = "Название товара";
		if (col == 2 && type == 2) res = "Сотрудник";
		if (col == 2 && type == 3) res = "Категория";
		return res;
	}
	private void fillCheck(int type){
		...
		jPanelMain.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(x + pl + mm_to_ppi(105), y, mm_to_ppi(65), h * 2));
		y = y + h * 2 + 5;
...
//						jPanelMain.add(jLabel46, new org.netbeans.lib.awtextra.AbsoluteConstraints(x + pl, y, mm_to_ppi(30)-1, h + 10));
					...
					jPanelMain.add(jLabel31, new org.netbeans.lib.awtextra.AbsoluteConstraints(x + pl, y, mm_to_ppi(170)-2, h+10));
					y = y + h + 10; 
					x = 0;	
...
//			System.out.println(mm_to_ppi(100) + c.getY() + 55);
			Dimension dim = Toolkit.getDefaultToolkit().getScreenSize();
			setSize(new Dimension(getWidth()+10, dim.height - 60));
    ...
    private void initComponents() {
        ...
        setMinimumSize(new java.awt.Dimension(795, 478));
        setModalityType(java.awt.Dialog.ModalityType.DOCUMENT_MODAL);
        setPreferredSize(new java.awt.Dimension(795, 478));
        ...
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                .addComponent(jPanelMenu, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addGap(0, 0, 0)
                .addComponent(jTabbedPane, javax.swing.GroupLayout.DEFAULT_SIZE, 446, Short.MAX_VALUE))
        );
