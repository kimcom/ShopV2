...
public class TmPriceOverContent extends AbstractTableModel{
    ...
    public TmPriceOverContent(ResultSet rs) {
        ...
        try {
            while (rs.next()) {
                Object[] row = {
                    ...
                    rs.getBigDecimal("PriceOver").setScale(2, RoundingMode.HALF_UP),
    ...
    public Object getValueAt(int rowindex, int columnindex) {
		...
		} else if (columnindex == 4) { //количество
            if (row[columnindex] == null) return "";
			BigDecimal bd = (BigDecimal) row[columnindex];
			if (bd.compareTo(BigDecimal.ZERO)>0) return "есть";
			return "нет";
